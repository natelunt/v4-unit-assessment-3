...
class Player extends Character{
  constructor(name, type, healthLevel, attackLevel){
   ...
   defend(attackLevel){
    ...
    this.healthLevel = this.healthLevel - attackLevel

       if(this.healthLevel > 0){
         return {attackStrength: this.attackLevel, remainingHealth: this.healthLevel, message: `${this.name} is still in the fight!`}
       } else {
...
fireSpitter.addSuperPower('regular spitting')
fireSpitter.addSuperPower('dry heaving')
